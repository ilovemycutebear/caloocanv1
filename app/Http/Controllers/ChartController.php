  ...
  $siteInfo = $requestc->sitename;
   $splitInfo = explode('||',$siteInfo, 2); // Restricts it to only 2 values, for names like Billy Bob Jones
    ...
    $splitId = $splitInfo[1];
      ...
      $data =  DB::Select("SELECT logs.site_id as label, logs.created_at as x, logs.rvalue as y FROM site inner join logs on site.id = logs.site_id where site.id = '".$splitId."' AND logs.created_at between '".$startDate."' and DATE_ADD( '".$endDate."', INTERVAL 1 DAY) order by logs.created_at asc");
  ...
  $siteInfo = $requestd->sitename;
   $splitInfo = explode('||',$siteInfo, 2); // Restricts it to only 2 values, for names like Billy Bob Jones
    ...
    $splitId = $splitInfo[1];
      ...
      $data =  DB::Select("SELECT logs.site_id as label, logs.created_at as x, logs.wlevel as y FROM site inner join logs on site.id = logs.site_id where site.id = '".$splitId."' AND logs.created_at between '".$startDate."' and DATE_ADD( '".$endDate."', INTERVAL 1 DAY) order by logs.created_at asc");
      $data = str_replace('label:', 'label:"', $data);
     $data = str_replace(',y:', '",y:', $data);
